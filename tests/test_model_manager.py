...
class TestMLXModelManager:
    ...
    def test_load_model_success(self, mock_exists, mock_is_dir, mock_validate, mock_load, mock_detect, model_manager, mock_model_and_tokenizer):
        ...
        assert str(model_manager.model_info.model_path).endswith("test/model/path")
    ...
    def test_load_model_path_not_exists(self, mock_exists, model_manager):
        ...
        assert "Invalid model path" in message
        assert model_manager.model_info is None
    ...
    @patch('openchat_mlx_server.model_manager.Path.is_dir')
    @patch('openchat_mlx_server.model_manager.Path.exists')
    def test_get_status_with_model(self, mock_exists, mock_is_dir, mock_validate, mock_load, model_manager, mock_model_and_tokenizer):
        ...
        # Mock the metadata detection to return test type
        with patch.object(model_manager, '_detect_model_metadata') as mock_metadata:
            ...
            assert status["path"].endswith("test/model/path")
